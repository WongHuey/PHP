...
else
{
    ...
    /* 文章 */
    $sql = "SELECT article_id,title,file_url,open_type FROM " .$ecs->table('article'). " WHERE is_open=1 and cat_id not in ('14','15')";
    $res = $db->query($sql);
    ...
    /* blog */
    $sql = "SELECT article_id,title,file_url,open_type FROM " .$ecs->table('article'). " WHERE is_open=1 and cat_id in ('14','15')";
    $res = $db->query($sql);

    while ($row = $db->fetchRow($res))
    {
        $article_url=$row['open_type'] != 1 ? build_uri('blog', array('aid'=>$row['article_id']), $row['title']) : trim($row['file_url']);
        $smi =& new google_sitemap_item($domain . $article_url,
            $today, $_POST['content_changefreq'], $_POST['content_priority']);
        $sm->add_item($smi);
    }
