...
elseif (isset($_REQUEST['category']))
{
	...
	//header("Location: https://www.kabeilu.com/404.php"); 
   // exit;

}

/* 代码增加_start  By xiaohua */
elseif(isset($_REQUEST['defurl']))
{

	$define_url=trim($_REQUEST['defurl']);
	$cat_id=$db->getOne("select cat_id from ". $ecs->table('category') ." where binary define_url ='$define_url' limit 0,1");
 ...
 * @return  array
 */
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order, $catid)
{
 ...
 $sql2 = "SELECT grade from ecs_category where cat_id=".$catid."";
 $nondis = $GLOBALS['db']->getOne($sql2);
...
" IFNULL(AVG(r.comment_rank),0) AS comment_rank,IF(r.comment_id,count(*),0) AS comment_count2,IF(r.comment_rank,count(*),0) AS comment_count, ".
'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img , g.keywords ' .
...
"WHERE $where $ext group by g.goods_id ORDER BY $sort $order";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$arr[$row['goods_id']]['zhekou_price2'] = round((1-($row['shop_price']/$row['market_price']))*100)/1+"";
		}	 
		 //小七修改结束
		...
		$arr[$row['goods_id']]['comment_count']=$row['comment_count2'];
		$arr[$row['goods_id']]['comment_rank']  = ceil($row['comment_rank']) == 0 ? 5 : ceil($row['comment_rank']);
		
		//$arr[$row['goods_id']]['goods_weight']          = (intval($row['goods_weight']) > 0) ? $row['goods_weight'] . $GLOBALS['_LANG']['kilogram'] : ($row['goods_weight'] * 1000) . $GLOBALS['_LANG']['gram'];
	
	//if ($catid!='66' && $catid!='71' && $catid!='44' && $catid!='78' && $catid!='74' && $catid!='27' && $catid!='27' && $catid!='27' && $catid!='27')
	if($nondis!=2)
	{
	$arr[$row['goods_id']]['goods_weight']   = "".intval(((intval($row['goods_weight']) > 0) ? $row['goods_weight']*1000  : ($row['goods_weight'] * 1000))/250*1);}
	else
	{
	$arr[$row['goods_id']]['goods_weight']   = "2.5";
	}
		
	
		
	//$arr[$row['goods_id']]['goods_ts']   = intval(((intval($row['goods_weight']) > 0) ? $row['goods_weight']*1000  : ($row['goods_weight'] * 1000))/250);	
	//$arr[$row['goods_id']]['goods_ts']   = intval(((intval($row['goods_weight']) > 0) ? $row['goods_weight']*1000  : ($row['goods_weight'] * 1000))/250);	
    }
