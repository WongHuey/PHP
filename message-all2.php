...
if ($action == 'act_add_message')
{
    ...
    /* 验证码防止灌水刷屏 */
    if ((intval($_CFG['captcha2']) & CAPTCHA_MESSAGE) && gd_version() > 0)
    {
        include_once('includes/cls_captcha.php');
        $validator = new captcha();
        if (!$validator->check_word($_POST['captcha2']))
        {
...
if(strstr($_POST['msg_content'],"<a")) 
{ 
echo "error/n"; 
exit();
}
if(strstr($_POST['msg_content'],"http")) 
{ 
echo "error/n"; 
exit();
}
if(strstr($_POST['msg_title'],"http")) 
{ 
echo "error/n"; 
exit();
}
	    ...
	    if (empty($_POST['msg_title']) || !empty($_SESSION['user_email']) || !empty($_SESSION['msg_content']))
    {
    ...
    $message = array(
        ...
        'msg_content' => isset($_POST['msg_content']) ? trim($_POST['msg_content'])." Mobile:".trim($_POST['user_mobile'])." URL:".trim($_POST['urlgood']) : '',
        'order_id'    => 0,
        'msg_area'    => 1,
        'upload'      => array()
     );
    ...
    {
        if (intval($_CFG['captcha2']) & CAPTCHA_MESSAGE)
        {
...
//getip
		...
		$to = '185287473@qq.com';
		//$to = '185287473@qq.com';
        $subject = '官网有一封询盘信息，请查看';
        $body = '时间：'.date('Y-m-d H:i:s').' 用户名：'.trim($_POST['msg_title']).' 国家：'.$countryCode.' 手机：'.trim($_POST['user_mobile']).' 邮箱：'.trim($_POST['user_email']).' 内容：'.trim($_POST['msg_content']).' URL：'.trim($_POST['urlgood']);
        mail($to, $subject, $body);

        show_message("Thank You! <br>We Will Contact You Within 6 Hours!");
?>


<script async src="https://www.googletagmanager.com/gtag/js?id=UA-117088198-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());
  gtag('config', 'UA-117088198-1');
</script>
...
if ($action == 'default')
{
    ...
    $smarty->assign('enabled_mes_captcha', (intval($_CFG['captcha2']) & CAPTCHA_MESSAGE));
