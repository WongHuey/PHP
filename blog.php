...
$cat_id02 = $db->getOne("SELECT cat_id FROM " . $ecs->table('article') . " WHERE article_id = '".$article_id."' ");
if($cat_id02!=15)
{
...
if (!$smarty->is_cached('blog.dwt', $cache_id))
{
    /* 文章详情 */
	...
	$smarty->assign('get_url',$row['open_type'] != 1 ? build_uri('blog', array('aid'=>$article_id), $article['title']) : trim($row['file_url']));
...
{
    $smarty->display('blog.dwt', $cache_id);
}
