...
function pay_fee($payment_id, $order_amount, $cod_fee=null)
{
    ...
    if (strpos($rate, '%') !== false)
    {
	   ...
	   //$pay_fee = round($val > 0 ? $order_amount * $val + 2.3 : 0);
	   $pay_fee = round(($val > 0 ? $order_amount * $val + 2.3 : 0),1);
...
//计算总价
	...
	//echo $total['shipping_fee'];		
		 //$total['shipping_fee']=136.00;	
	$allcount=$weight_price['weight']*1000/250;
	
		
		//echo $shipping_info['shipping_area_id'];
		
		
	if($shipping_info['shipping_area_id']==8)
	{	

    $sql2 = 'SELECT area1 from shipping_money where id='.$allcount.'';
    $row = $GLOBALS['db']->getRow($sql2);

    if (!empty($row))
    {
	$total['shipping_fee']=$row["area1"];
	}
	
	}
		 
		 
	if($shipping_info['shipping_area_id']==11)
	{	

    $sql2 = 'SELECT area2 from shipping_money where id='.$allcount.'';
    $row = $GLOBALS['db']->getRow($sql2);

    if (!empty($row))
    {
	$total['shipping_fee']=$row["area2"];
	}
	
	}
	
	
	if($shipping_info['shipping_area_id']==12)
	{	

    $sql2 = 'SELECT area3 from shipping_money where id='.$allcount.'';
    $row = $GLOBALS['db']->getRow($sql2);

    if (!empty($row))
    {
	$total['shipping_fee']=$row["area3"];
	}
	
	}
	
	
	 if($shipping_info['shipping_area_id']==16)
	{	

    $sql2 = 'SELECT area5 from shipping_money where id='.$allcount.'';
    $row = $GLOBALS['db']->getRow($sql2);

    if (!empty($row))
    {
	$total['shipping_fee']=$row["area5"];
	}
	...
	if($shipping_info['shipping_area_id']==14)
	{	

    $sql2 = 'SELECT area4 from shipping_money where id='.$allcount.'';
    $row = $GLOBALS['db']->getRow($sql2);

    if (!empty($row))
    {
	$total['shipping_fee']=$row["area4"];
	}
...
if ($total['goods_price']*0.003<8)
{
$total['insurance'] = "8";
}
else
{
$total['insurance'] = round($total['goods_price']*0.003,2);
}
