...
function insert_cart_info2()
{
    ...
    return '<a href="flow.php" title="' . $GLOBALS['_LANG']['view_cart'] . '"><font color=red>(' . $number .")<font class='ycmobile'> ". price_format($amount, false) . '</font></font></a>';
}
...
$ip = $_SERVER["REMOTE_ADDR"];
		  ...
		  // echo $ip_json;
           if($ip_arr['code']==0)
           {
		     // echo $ip_arr['data']['country_id'];
			  
if($ip_arr['data']['country_id']=="CN" && $arr[note]=="")
{
	//echo "<font color='blue'> $ip 在国内</font>";
	return '';

}
else if($ip_arr['data']['country_id']=="CN" && $arr[note]!="")
{
	//echo "<font color='blue'> $ip 在国内</font>";
