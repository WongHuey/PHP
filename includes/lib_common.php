...
function build_uri($app, $params, $append = '', $page = 0, $keywords = '', $size = 0)
{
    ...
    switch ($app)
    {
        ...
        case 'blog':
            ...
            {
                $uri = $rewrite ? 'blog-' . $aid : 'article.php?id=' . $aid;
            }
...
if($app="category" && empty($define_url))
{
$uri .= '-' . strtolower(urlencode(str_replace(' ','-',preg_replace('/[\.|\/|\?|&|\+|\\\|\'|"|,]+/', '', $append))));
...
$uri .= '.html';
				    ...
				    if($uri=="category-2-b0-Brazilian+Hair.html")
					{
					$uri = "Brazilian-Hair";
					}
					
					
				    if($uri=="category-6-b0-Indian+Hair.html")
					{
					$uri = "Indian-Hair";
					}
					
				     if($uri=="category-1-b0-Peruvian+Hair.html")
					{
					$uri = "Peruvian-Hair";
					}
					
				     if($uri=="category-8-b0-Malaysian+Hair.html")
					{
					$uri = "Malaysian-Hair";
					}
					
					
				     if($uri=="category-10-b0-Closure+Hair.html")
					{
					$uri = "Lace-Closure";
					}
					
				    if($uri=="category-38-b0-7A+Hair.html")
					{
					$uri = "7A-Hair";
					}
					if($uri=="category-44-b0-Bundles++Closure.html")
					{
					$uri = "Bundles-Closure";
					}
