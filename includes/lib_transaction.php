...
function get_order_detail($order_id, $user_id = 0)
{
        ...
        $order['order_status'] == OS_CONFIRMED))
    {
        ...
        else
        {
			...
			$country20 = $GLOBALS['db']->GetOne("SELECT region_name FROM " . $GLOBALS['ecs']->table('region') . " WHERE region_id = ".$order['country']);
			

if ($order['goods_amount']*0.003<8)
{
$insurance = "8";
}
else
{
$insurance = round($order['goods_amount']*0.003,2);
}

 $order['insurance'] = $insurance;

			
            $order['pay_online'] = $pay_obj->get_code2($order, $insurance,$payment,$country20,$order['province'],$order['city'],$order['zipcode'],$order['email'],$order['consignee'],$order['address']);
        }
    }
    else
    {
