...
"IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type,g.goods_weight, " .
...
"WHERE $where $ext group by g.goods_id ORDER BY $sort $order";
	...
	$arr[$row['goods_id']]['goods_weight']          = "".intval(((intval($row['goods_weight']) > 0) ? $row['goods_weight']*1000  : ($row['goods_weight'] * 1000))/250*100)."g";
