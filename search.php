...
else
{
    ...
    $sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
                ...
                "g.promote_price, g.promote_start_date, g.promote_end_date, g.goods_thumb, g.goods_img, g.goods_weight, g.goods_brief, g.goods_type, g.goods_number, g.cat_id ".
    ...
    while ($row = $db->FetchRow($res))
    {
		...
		//$arr[$row['goods_id']]['goods_weight']          = "".intval(((intval($row['goods_weight']) > 0) ? $row['goods_weight']*1000  : ($row['goods_weight'] * 1000))/250)."";
		
		
		 $sql2 = "SELECT grade from ecs_category where cat_id=".$row['cat_id']."";
