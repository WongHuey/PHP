...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
        ...
        $smarty->assign('get_url',build_uri('goods', array('gid'=>$goods_id), $goods['goods_style_name']));
        ...
        assign_template('c', $catlist);
         ...
         /* 上一个商品下一个商品 */
        $prev_gid = $db->getOne("SELECT goods_id,goods_name FROM " .$ecs->table('goods'). " WHERE cat_id=" . $goods['cat_id'] . " AND goods_id > " . $goods['goods_id'] . " AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0 LIMIT 1");
        if (!empty($prev_gid))
        {
			$prev_good = $db->getRow("SELECT goods_id,goods_name FROM " .$ecs->table('goods'). " WHERE  goods_id = $prev_gid");
            $prev_good['url'] = build_uri('goods', array('gid' => $prev_gid),  $prev_good['goods_name']);
        ...
        if (!empty($next_gid))
        {
			$next_good = $db->getRow("SELECT goods_id,goods_name FROM " .$ecs->table('goods'). " WHERE  goods_id = $next_gid");
